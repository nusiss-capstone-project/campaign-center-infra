...
usage() {
  ...
  echo "  HEADLAMP_HOST         Hostname (default: headlamp.<master_public_ip>.sslip.io)"
  echo "  HEADLAMP_TLS          true to use Traefik websecure + TLS secret (default: false)"
  echo "  HEADLAMP_TLS_SECRET   TLS secret in headlamp ns (default: campaignhub-origin-tls)"
  echo "  HEADLAMP_SKIP_HELM    true to skip Helm (only RBAC + IngressRoute); use when chart fetch hangs"
  echo ""
  echo "Prerequisites: K3s installed, Traefik configured (install-traefik.sh)"
  echo "For HTTPS with campaignhub.best: run configure-origin-tls.sh first, then:"
  echo "  HEADLAMP_HOST=headlamp.campaignhub.best HEADLAMP_TLS=true HEADLAMP_SKIP_HELM=true \\"
  echo "    $(basename "$0") dev"
  echo ""
